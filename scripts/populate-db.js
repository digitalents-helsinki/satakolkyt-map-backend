...
async function populateDB({ geojson, collectionName }) {
  ...
  try {
    ...
    const stepskm = connection.collection('steps_km_info')
    await stepskm.create().catch(err => {
      if (err.code === 409) {
        console.log(`Found existing collection 'steps_km_info'. `)
        return
      }
